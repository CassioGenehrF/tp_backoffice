                ...
                @foreach ($filters as $title => $filter)
                    <div class="accordion-item">
                        <h2 class="accordion-header" id="heading{{ Str::slug($title) }}">
                            <button class="accordion-button {{ $filter['status'] ? '' : 'collapsed' }}" type="button"
                                data-bs-toggle="collapse" data-bs-target="#{{ Str::slug($title) }}"
                                aria-expanded="{{ $filter['status'] }}" aria-controls="{{ Str::slug($title) }}">
                                {{ $title }}
                            </button>
                        </h2>
